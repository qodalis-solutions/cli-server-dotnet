...
{
    private readonly SqliteFileStorageProvider _provider;
    ...
    [Fact]
    public void Name_ReturnsSqlite()
    {
    ...
    [Fact]
    public void UseSqlite_SetsProvider()
    {
        var options = new FileSystemOptions();
        options.UseSqlite(":memory:");

        Assert.NotNull(options.Provider);
        Assert.IsType<SqliteFileStorageProvider>(options.Provider);
        Assert.Equal("sqlite", options.Provider.Name);
    }
}
